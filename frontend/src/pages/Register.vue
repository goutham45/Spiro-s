...
export default {
    ...
    methods: {
        ...
        checkForm: function () {
            ...
            // Phone validate (US format)
            if (!this.registerObj.phone) {
                this.errorObj.phoneErr.push('Phone number is required');
            } else {
                const digits = this.registerObj.phone.replace(/\D/g, '');
                if (digits.length < 10 || digits.length > 11) {
                    this.errorObj.phoneErr.push('Please enter a valid US phone number');
                }
            }
            ...
            if (!this.checkEmptyErr()) {
                ...
                try {
                    ...
                    if (msg.toLowerCase().includes("email")) {
                        this.errorObj.emailErr.push(msg);
                    } else {
                        this.errorObj.nameErr.push(msg);
                    }
                }
            }
        }
